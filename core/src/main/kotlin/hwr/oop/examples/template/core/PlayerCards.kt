package hwr.oop.examples.template.core

private val initialStock = List(4){ CardID.COPPER } + List(3){ CardID.ESTATE }

class PlayerCards(private val stock: List<CardID> = initialStock,
                  private val discard: List<CardID> = emptyList(),
                  private val hand: List<CardID> = emptyList(),
                  private val used: List<CardID> = emptyList())
{

    fun discard(): PlayerCards {
        return PlayerCards(stock, discardedCards())
    }

    private fun discardedCards() = discard + hand + used

    fun draw(count: Int): PlayerCards {
        val remaining: Int = stock.size - count
        val drawn = stock.takeLast(count)
        if (remaining >= 0) {
            return drawFromStock(drawn, remaining)
        }

        val missing = remaining * -1
        if (discard.size < missing) {
            return drawAsManyAsPossible(drawn)
        }

        return reshuffleAndDraw(drawn, missing)
    }

    private fun drawnCards(drawn: List<CardID>) = hand + drawn

    private fun drawFromStock(drawn: List<CardID>, remaining: Int): PlayerCards {
        val remainder = stock.take(remaining)
        return PlayerCards(remainder, discard, drawnCards(drawn), used)
    }

    private fun drawAsManyAsPossible(drawn: List<CardID>): PlayerCards {
        return PlayerCards(emptyList(), emptyList(), drawnCards(drawn) + discard, used)
    } // this means the player has fewer cards then he is trying to draw which IS a valid case

    private fun reshuffleAndDraw(drawn: List<CardID>, missing: Int): PlayerCards{
        val refill = PlayerCards(discard.shuffled(), emptyList(), drawnCards(drawn))
        return refill.draw(missing)
    }

    fun extractActiveCards() = ActiveCards(hand, used)
    fun insertTurnState(hand: List<CardID>, used: List<CardID>): PlayerCards {
        return PlayerCards(stock, discard, hand, used)
    }

   fun insert(card: CardID): PlayerCards {
       return PlayerCards(stock, discard + card, hand)
   }

    fun handSize() = hand.size
    fun usedSize() = used.size
    fun stockSize() = stock.size
    fun discardSize() = discard.size

}
